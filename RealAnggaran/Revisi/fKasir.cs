...
namespace RealAnggaran.Revisi
{
   ...
   */
    ...
    public partial class fKasir : Form
    {
        ...
        int modePilihKpa = 0; // mode yg terjadi apabila user telah memilih pada combobox KPA
        ...
        private decimal CekSisaAnggaran()
        {
            ...
            {
                reader.Read();
                //if ((Convert.ToDecimal(alat.pengecekField(reader, 0)) - totalBayar) < 0) // saldo tidak mencukupi
                //{
                ...
                //    i = 1;
                //}
                i = (Convert.ToDecimal(alat.PengecekField(reader, 0)) - totalBayar);
                reader.Close();
            }
            else
        ...
        private void bSave_Click(object sender, EventArgs e)
        {
            ...
            //{
                if (bSave.Text == "SIMPAN")
                {
                    ...
                    else if (modePilihKpa == 0 | txtNoKwitansi.Text.Contains(".."))
                    {
                        MessageBox.Show("Silahkan Pilih KPA dan lengkapi Nomor Kwitansi", "PERHATIAN");
                        comboBox1.Focus();
                    }
                    ...
                    {
                        MessageBox.Show("No Kwitansi/Bukti sudah pernah dipakai.\nSilahkan pilih nomor lainnya", "PERHATIAN");
                        txtNoKwitansi.Focus();
                    }
                    else
                    {
        ...
        private void SimpanData(int statusTrans)
        {
            ...
            {
                MessageBox.Show("disini" + e.Message);
                tran.Rollback();
            }
            finally
            {
        ...
        private void InsertTrans(SqlTransaction trans)
        {
            query = @"INSERT INTO A_PENGELUARAN (TglBayar, NoBayar, Id_Reken, Id_Supplier, PPnRp, PPhRp, JmlRp, Lunas, " +
                ...
                "', '" + txtNoPPh.Text.Trim() + "', '" + txtNoSPK.Text + "', GETDATE(), GETDATE(), " + idKPA + ")";
        ...
        private void UpdateTrans(SqlTransaction trans)
        {
            query = @"UPDATE REALANGGAR.dbo.A_PENGELUARAN SET PPnRp = " + filtertxt(txtPPn.Text.Trim()) + ", PPhRp = " + filtertxt(txtPPh.Text.Trim()) +
                ...
                ", NoBayar = '" + txtNoKwitansi.Text + "', Wkt_Update = GETDATE(), Id_Kpa = " + idKPA + " WHERE No_SPK = '" + txtNoSPK.Text.Trim() + "'" +
                ...
                //"', 103), ket = '" + txtNoKwitansi.Text + "', Wkt_Update = GETDATE() WHERE ket = '" + lNoBukti.Text.Trim() + "'" +
        ...
        {
            modePilihKpa = 0; // mode yg terjadi apabila user belum memilih pada combobox KPA
            ...
            //", REALANGGAR..A_PENGELUARAN E, REALANGGAR..A_PENERIMAAN F, REALANGGAR..A_KPA G " +
            //"WHERE A.No_SPK LIKE '%" + lvTampil.SelectedItems[0].SubItems[1].Text + "%' AND B.IdBlj_Master = A.IdBlj_Master AND C.Kd_Reken = B.Id_Rinci_Rs AND D.KdSupp = A.IdSupplier " +
            //"AND ((E.No_SPK = A.No_SPK) AND (G.id_Kpa = F.id_Kpa AND F.ket = E.NoBayar)) " +
            //"ORDER BY A.Tgl_SP";
            //if (!backgroundWorker1.IsBusy)
            //{
            //MessageBox.Show("Test1");
            koneksi.Open();
            reader = konek.MembacaData("SELECT DISTINCT A.No_SPK, A.Tgl_SP, A.Keter, B.Id_Rinci_Rs, CONVERT(varchar(1000),C.Uraian) AS 'uraian', D.NamaSupp, B.TFungsi, " +
                "B.TSubsi AS 'ANEH', A.Lunas, A.TGL_SPJ, A.No_Bukti, C.Id_Reken, D.id_Supplier " +
                ", E.NTPNPPn, E.NTPNPPh, E.PPhNo, G.Id_Kpa, G.KdKpa, E.noBayar " +
                "FROM KASDA..BLJ_MASTER A, KASDA..BLJ_DETAIL B, REALANGGAR..A_REKENING C, REALANGGAR..A_SUPPLIER D " +
                ", REALANGGAR..A_PENGELUARAN E, REALANGGAR..A_PENERIMAAN F, REALANGGAR..A_KPA G " +
                "WHERE A.No_SPK LIKE '%" + lvTampil.SelectedItems[0].SubItems[1].Text + "%' AND B.IdBlj_Master = A.IdBlj_Master AND C.Kd_Reken = B.Id_Rinci_Rs AND D.KdSupp = A.IdSupplier " +
                "AND ((E.No_SPK = A.No_SPK) AND (G.id_Kpa = F.id_Kpa AND F.ket = E.NoBayar)) " +
                "ORDER BY A.Tgl_SP", koneksi);
            //MessageBox.Show(query);
            if (reader.HasRows)
            {
                reader.Read();
                //MessageBox.Show("Test2");
                txtNTPNPPn.Text = alat.PengecekField(reader, 13);
                txtNTPNPPh.Text = alat.PengecekField(reader, 14);
                txtNoPPh.Text = alat.PengecekField(reader, 15);
                idKPA = Convert.ToInt16(alat.PengecekField(reader, 16));
                comboBox1.Text = alat.PengecekField(reader, 17);
                txtNoKwitansi.Text = alat.PengecekField(reader, 18);
        ...
        private void txtPPn_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("PPn error" + ex.Message);
                    return;
                }
            }
            temp = "0";
        ...
        private string generateNoKwitansi()
        {
            ...
            //    " ORDER BY A DESC";
            query = "SELECT COUNT (Id_Bayar_Mst) FROM A_PENGELUARAN WHERE datepart(year, tglBayar) = '" + dateTimePicker1.Text.Substring(6, 4) + "' " +
                " AND datepart(month, tglBayar) = '" + dateTimePicker1.Text.Substring(3, 2) +
                "' AND substring(NoBayar, 1,2) = '" + comboBox1.Text.Right1(2) + "'";
            ...
            if (reader.HasRows)
            {
                ...
                try
                {
                    ...
                    noBukti = comboBox1.Text.Right1(2) + "/" + dateTimePicker1.Text.Substring(3, 2) + "/" + (Convert.ToInt16(alat.PengecekField(reader, 0)) + 1).ToString();
                }
                catch (Exception ex)
                {
        ...
        */
        ...
        private bool cek_saldo()
        {
            ...
            koneksi.Close();
            MessageBox.Show(idSumber.ToString() + "" + idKPA.ToString());
            MessageBox.Show(sisaAnggaran.ToString() + " " + totalBayar.ToString());
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (statusShowTable == 0) // Tanpa filter
            {
                ...
                query = "SELECT DISTINCT A.No_SPK, A.Tgl_SP, A.Keter, B.Id_Rinci_Rs, CONVERT(varchar(1000),C.Uraian) AS 'uraian', D.NamaSupp, B.TFungsi, " +
                    ...
                    "AND (G.id_Kpa = F.id_Kpa AND F.ket = E.NoBayar) " +
                    "ORDER BY A.Tgl_SP";
            else if (statusShowTable == 9) // event txtNoSPK keyPressed
                /*query = "SELECT DISTINCT A.No_SPK, A.Tgl_SP, A.Keter, B.Id_Rinci_Rs, CONVERT(varchar(1000),C.Uraian) AS 'uraian', D.NamaSupp, B.TFungsi, " +
                    "B.TSubsi AS 'ANEH', A.Lunas, A.TGL_SPJ, A.No_Bukti, C.Id_Reken, D.id_Supplier " +
                    ", E.NTPNPPn, E.NTPNPPh, E.PPhNo, G.Id_Kpa, G.KdKpa " +
                    "FROM KASDA..BLJ_MASTER A, KASDA..BLJ_DETAIL B, REALANGGAR..A_REKENING C, REALANGGAR..A_SUPPLIER D " +
                ...
                query = "SELECT DISTINCT A.No_SPK, A.Tgl_SP, A.Keter, B.Id_Rinci_Rs, CONVERT(varchar(1000),C.Uraian) AS 'uraian', D.NamaSupp, B.TFungsi, " +
                    ...
                    "AND ((E.No_SPK = A.No_SPK) AND (G.id_Kpa = F.id_Kpa AND F.ket = E.NoBayar)) " +
                    "ORDER BY A.Tgl_SP";
            // hitung record start here
            koneksi.Open();
            int jumlahRecord = 0;
            reader = konek.MembacaData("SELECT COUNT (A.No_SPK) " +
                    "FROM KASDA..BLJ_MASTER A, KASDA..BLJ_DETAIL B, REALANGGAR..A_REKENING C, REALANGGAR..A_SUPPLIER D " +
                    "WHERE B.IdBlj_Master = A.IdBlj_Master AND C.Kd_Reken = B.Id_Rinci_Rs AND D.KdSupp = A.IdSupplier", koneksi);
            //int jumlahRecord =  int.Parse(alat.pengecekField(reader, 0)); //
            if (reader.HasRows)
            {
                reader.Read();
                int.TryParse(alat.PengecekField(reader, 0), out jumlahRecord);
                reader.Close();
                progressBar1.SafeControlInvoke(progressBar => 
                    {
                        progressBar1.Visible = true;
                        progressBar1.Maximum = jumlahRecord;
                    });
            }
            koneksi.Close();
            // hitung record stop here
            Thread.Sleep(100);
            koneksi.Open();
            reader = konek.MembacaData(query, koneksi);
            if (reader.HasRows)
        ...
        private void txtPPh_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("PPh error " + ex.Message);
                    return;
                }
            }
        }
        ...
        private void txtNoKwitansi_TextChanged(object sender, EventArgs e)
        {
            ...
            modePilihKpa = 1; // mode yg terjadi apabila user telah memilih pada combobox KPA
        ...
        private void txtNoKwitansi_Enter(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtNoKwitansi.Text))
            {
                if (statusGenerateYesorNo == 1)
                {
                    txtNoKwitansi.Focus();
                    txtNoKwitansi.Select(generateNoKwitansi().Length + 1, 80);
                }
            }
        }

        private void txtNoKwitansi_MouseClick(object sender, MouseEventArgs e)
        {
            if (!string.IsNullOrEmpty(txtNoKwitansi.Text))
            {
                if (statusGenerateYesorNo == 1)
        ...
        int statusGenerateYesorNo = 0; // status mark if value in txtNoKwitansi is generated or not
            ...
            if (comboBox1.Text.Trim() != "")
            {
                if (isInEditMode == true)
                {
                    if (lvTampil.SelectedItems[0].SubItems[9].Text.Trim() != "Y")
                    {
                        ...
                        idKPA = Convert.ToInt16(listku[0].ToString());
                        txtNoKwitansi.Text = generateNoKwitansi() + "/........";
                        statusGenerateYesorNo = 1;
                    }
                }
            }
        }

        private void txtNoKwitansi_Leave(object sender, EventArgs e)
        {
            statusGenerateYesorNo = 0;
        }
    }
